...
| T7 | Datapatch uppdrag.json | data/uppdrag.json | ✅ Done (1 av 2 uppdrag; 1 avvaktar) | **Digitala prov** (2026-07-24) infört under betyg.relaterade med källbelagda fält (titel/datum/URL ur A.3); fulltext hänvisar till regeringsbeslutet — A.3 gav ej uppdragstextens innehåll (sandbox-nätblock), ingen fabricering. **Införandestöd** (2026-06-23): AVVAKTAR — A.3 innehåller inget utdrag som anger vilka reformnoder uppdragstexten kopplar till, och uppdrag.json kräver ett reform_id för placering → RAPPORTERAT, kan ej patchas utan nodlista från Niklas. **Statsbidrag yrkesvux**: AVFÖRD per rev B (statsbidragsförordning för regionalt yrkesvux, utanför reformspårningen — ingen patch, ingen kontroll). JSON validerar, testsvit 61/61. |
